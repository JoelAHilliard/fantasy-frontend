    ...
    return (   
        <div className='mt-2'>
            {/* week and year buttons */}
             <div className="relative inline-flex flex-col text-left justify-center ml-2">
                ...
                <button disabled={year == ""} onClick={() => setIsWeekOpen(!isWeekOpen)} className="w-full inline-flex justify-center py-2 px-4 text-sm font-medium text-white bg-green-700 border border-transparent rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 whitespace-nowrap disabled:cursor-not-allowed disabled:bg-green-700/50">
          ...
          <div className="flex flex-col rounded-md shadow-lg bg-green-50 ring-1 ring-black ring-opacity-5 z-100">
              <div className="flex flex-col mt-1" style={{'position':'absolute','width':'40%','alignSelf':'end'}} role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                  ...
                  {data[0].year == 2017 ? weeks.slice(0,15).map(week => (
                    <a onClick={()=>{setWeek(week);}} className="px-4 py-2 text-sm text-green-700 hover:bg-green-400 hover:text-green-900 bg-green-200 text-center cursor-pointer" role="menuitem">Week {week}</a>
                  )):  weeks.map(week => (
                    <a onClick={()=>{setWeek(week);}} className="px-4 py-2 text-sm text-green-700 hover:bg-green-400 hover:text-green-900 bg-green-200 text-center cursor-pointer" role="menuitem">Week {week}</a>
                  ))}  
              </div>
            ...
            {!!data ? data.map(data =>(
                <div>
                    ...
                    <div className="p-4">
                        ...
                        <div className="grid grid-cols-2 gap-5 text-center bg-green-100 rounded-lg">
                            {/* Team Headers */}
                            ...
                            {data.home_team_lineup !== "N/A" ?
                                data.home_team_lineup.map((player, index) => (
                                <div key={player.id || index}>
                                    <div className="p-2 border-t border-green-200">
                                    {player.name}: {player.points}
                                    </div>
                                    {data.away_team_lineup[index] && (
                                    <div className="p-2 border-t border-green-200">
                                        {data.away_team_lineup[index].name} - {data.away_team_lineup[index].points}
                                    </div>
                                    )}
                                </div>
                                ))
